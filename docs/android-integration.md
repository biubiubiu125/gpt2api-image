...
> [!NOTE]
> **官方 APK 闭源 / 后端 API 开源。**
> - 官方维护的 Draw 安卓客户端只在 [Releases](https://github.com/RemotePinee/gpt2api-image/releases) 以 APK 形式发布，本仓库不包含其源码。
...
| 包管理 | uv |
| Web 框架 | FastAPI |
| 后端 repo | https://github.com/RemotePinee/gpt2api-image |
| API 版本 | 见 `VERSION` 文件 |
...
| API 服务密钥 | 管理员在后台 `/api/auth/users` 创建的 `sk-...` 形式密钥，按管理员级服务密钥处理 | 给 newapi 或其它下游服务调用 |

后端识别这两种来自同一个 header，AP 端无需区分。当前服务密钥会规范化为管理员级服务密钥，不走普通用户额度模型；如果要隔离不同下游，建议使用不同密钥并在外层系统做审计和限流。
...
访问密钥 (Auth Key)：admin auth_key 或 sk-... API 服务密钥
```
...
```json
{
  "object": "list",
  "data": [
    { "id": "gpt-image-2", "object": "model", "owned_by": "gpt2api-image" }
  ]
}
```
...
- `/v1/models` 对外只暴露公开画图模型 `gpt-image-2`。
- AP 端不需要展示模型下拉，直接固定传 `gpt-image-2`；即使兼容传入 `gpt-image-1`、`dall-e-3`、`codex-gpt-image-2` 或空值，后端也会统一归一化为 `gpt-image-2`。
- 真实上游链路不由外部 `model` 字段决定，而由后端 `image_route_strategy` 配置决定：默认 `web_first` 优先走 ChatGPT Web 网页逆向生图，Web 不可用或临时失败时再尝试 Codex；如只想使用网页链路，可设置为 `web_only`。
...
| `resolution` | string |   | 支持 `1k` `2k` `4k`。用于超清 Prompt 增强和 Codex 内部 size 映射 |
| `response_format` | string |   | `url` 或 `b64_json`，默认 `b64_json`。**安卓端强烈建议 `url`**（解 b64 慢且耗内存） |
| `stream` | bool |   | 见下文流式说明 |

**清晰度与账号池调度**：

- 每个生图任务都会在原始 prompt 后自动追加超清图片约束，`resolution=1k/2k/4k` 会追加对应清晰度目标。
- 真实账号池调度由后端 `image_route_strategy` 决定，不再因为 `resolution=2k/4k` 强制切到 Codex。
- 当内部路由实际走 Codex 时，`4k + size=9:16` 会映射到 `2160x3840`；`4k + size=16:9` 会映射到 `3840x2160`；其它画幅按同等长边规则映射。
...
```json
{
  "identity": {
    ...
    "image_daily_remaining": null,
    "image_monthly_quota": 0,
    "image_monthly_used": 0,
    "image_monthly_unlimited": true,
    "image_monthly_remaining": null,
    "image_total_quota": 0,
    "image_total_used": 0,
    "image_total_unlimited": true,
    "image_total_remaining": null
  }
}
```

当前主密钥和后台创建的 `sk-...` 服务密钥都会按管理员级服务密钥返回，六档额度为 `*_unlimited: true`、`*_remaining: null`。AP 端按"无限额度"展示即可，真实瓶颈在后端账号池可用量。

**身份能力字段**：

| 字段 | 含义 |
|---|---|
| `account_tier=premium` | 服务密钥默认返回高级能力 |
| `can_use_paid_image_accounts=true` | 当前密钥可使用内部付费账号链路 |
| `can_use_high_resolution=true` | AP 端可展示 2K / 4K 选项 |

> UI 禁用只是体验。后端仍会在 `/v1/images/*` 等入口强制校验权限字段。
...
- 服务密钥：按当前 identity.id 返回自己生成的图；主密钥会归入 `__admin__`
- admin 视角：图片管理后台可按来源密钥筛选，便于区分不同下游渠道
...
| 403 | 权限不足（发布别人的图、非 admin 调 admin 接口、密钥无权使用高分辨率等） | n/a | 提示具体 `error`，不重试 |
...
| P1 | API 服务密钥 | ✅ 已实现。后台创建的 `sk-...` 密钥会按管理员级服务密钥处理，不走普通用户额度模型 |
| P1 | `/v1/images/*` 接受 `resolution=1k/2k/4k` | ✅ 已实现。清晰度参数会进入超清 Prompt 增强；真实 Web/Codex 路由由 `image_route_strategy` 决定 |
...
# 4K 竖图。外部模型仍是 gpt-image-2；真实 Web/Codex 路由由后端 image_route_strategy 决定
curl -X POST http://127.0.0.1:8000/v1/images/generations \
  -H "Authorization: Bearer YOUR_KEY" \
  -H "Content-Type: application/json" \
...
| 日期 | 版本 | 改动 |
|---|---|---|
| 2026-05-22 | v1.2.2 | 新增画廊接口（feed/publish/unpublish/published/batch）+ `/api/me/images` 章节；鉴权节加 IOException / 启动 me() 体检策略与 skip-unauth-hook 头使用；图片清理引入 `cleanup_protect_gallery` / `cleanup_protect_user_images` 两个保护开关；生图链路统一上游真失败 / 取消自动退还预扣额度；错误码表加退额度列；澄清外部画图模型统一为 `gpt-image-2` |
