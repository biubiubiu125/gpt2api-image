...
func (s *Server) ensureCurlImpersonateBinary() (string, error) {
	if bin := strings.TrimSpace(os.Getenv("GPT2API_IMAGE_CURL_IMPERSONATE_BIN")); bin != "" {
		if st, err := os.Stat(bin); err == nil && !st.IsDir() {
			return bin, nil
		}
		return "", fmt.Errorf("GPT2API_IMAGE_CURL_IMPERSONATE_BIN not found: %s", bin)
	}
	...
	if strings.EqualFold(strings.TrimSpace(os.Getenv("GPT2API_IMAGE_CURL_IMPERSONATE_AUTO_DOWNLOAD")), "0") || strings.EqualFold(strings.TrimSpace(os.Getenv("GPT2API_IMAGE_CURL_IMPERSONATE_AUTO_DOWNLOAD")), "false") {
...
func resolveCurlImpersonateAsset() (string, string, error) {
	if explicit := strings.TrimSpace(os.Getenv("GPT2API_IMAGE_CURL_IMPERSONATE_URL")); explicit != "" {
