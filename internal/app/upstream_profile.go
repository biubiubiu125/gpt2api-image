...
func upstreamTLSProfile() profiles.ClientProfile {
	name := strings.ToLower(strings.TrimSpace(os.Getenv("GPT2API_IMAGE_TLS_PROFILE")))
	if name == "" {
...
func upstreamUserAgent() string {
	if ua := strings.TrimSpace(os.Getenv("GPT2API_IMAGE_USER_AGENT")); ua != "" {
		return ua
	}
...
func upstreamSecCHUA() string {
	if chua := strings.TrimSpace(os.Getenv("GPT2API_IMAGE_SEC_CH_UA")); chua != "" {
		return chua
	}
	profile := strings.ToLower(strings.TrimSpace(os.Getenv("GPT2API_IMAGE_TLS_PROFILE")))
	version := "143"
