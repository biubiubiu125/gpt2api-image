...
func (s *Server) handleV1Models(w http.ResponseWriter, r *http.Request) {
	...
	for _, m := range s.imageModelIDs() {
		data = append(data, modelListItem(m, created))
	}
...
func (s *Server) handleV1ChatCompletionsImageOnly(w http.ResponseWriter, r *http.Request) {
	...
	if !isImageChatRequest(b) {
		writeErr(w, 404, "text chat API is disabled in gpt2api-image")
		return
	}
...
func (s *Server) handleV1ResponseImage(w http.ResponseWriter, r *http.Request, id *Identity, b map[string]any, callID string) {
	...
	if s.taskStore != nil {
		...
		s.logCallSuccess(callID, "/v1/responses", model, "Responses", map[string]any{"task_id": task.ID, "queued": true})
		writeJSON(w, 202, responseTaskResponse(task))
		return
	}
