...
# gpt2api-image latest installer/updater
#
# Default behavior:
...
#   bash scripts/install_latest.sh --repo jwbb903/gpt2api-image --dir ./app

REPO="${GPT2API_IMAGE_INSTALL_REPO:-jwbb903/gpt2api-image}"
INSTALL_DIR="${GPT2API_IMAGE_INSTALL_DIR:-$PWD/gpt2api-image-install}"
PORT="${GPT2API_IMAGE_INSTALL_PORT:-3000}"
FROM_SOURCE=0
BUILD_WEB=0
SKIP_TESTS=0
...
Options:
  --repo OWNER/REPO      GitHub repository. Default: jwbb903/gpt2api-image
  --dir PATH            Install directory. Default: ./gpt2api-image-install
  ...
  --skip-tests          Skip Go backend tests when using --from-source.
  --start               Start service after install by exec'ing start.sh.
  --force               Continue even if install dir has unknown files.
  -h, --help            Show this help.

Environment:
  GPT2API_IMAGE_INSTALL_REPO   Same as --repo.
  GPT2API_IMAGE_INSTALL_DIR    Same as --dir.
...
check_install_dir() {
  ...
  if [[ "$FORCE" -eq 0 && ! -f "$INSTALL_DIR/start.sh" && ! -f "$INSTALL_DIR/gpt2api-image" && ! -f "$INSTALL_DIR/gpt2api-image" ]]; then
    local count
    count="$(find "$INSTALL_DIR" -mindepth 1 -maxdepth 1 | wc -l | tr -d ' ')"
    if [[ "$count" != "0" ]]; then
      fatal "$INSTALL_DIR is not empty and does not look like a gpt2api-image install dir. Use --force to continue."
    fi
  fi
}
...
install_from_release() {
  ...
  local asset="gpt2api-image-${os}-${arch}.tar.gz"
  ...
  chmod +x "$INSTALL_DIR/gpt2api-image" 2>/dev/null || true
  chmod +x "$INSTALL_DIR/gpt2api-image" 2>/dev/null || true
...
install_from_source() {
  ...
  mkdir -p bin
  CGO_ENABLED=0 go build -buildvcs=false -trimpath -ldflags='-s -w' -o bin/gpt2api-image ./cmd/server

  check_install_dir
  backup_existing_state "$backup_dir"
  rm -rf "$INSTALL_DIR"
  mkdir -p "$INSTALL_DIR/bin"
  cp bin/gpt2api-image "$INSTALL_DIR/bin/gpt2api-image"
