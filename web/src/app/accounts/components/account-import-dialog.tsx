...
type ImportMethod = "menu" | "token" | "session" | "json";
...
function getAccountAccessToken(value: unknown) {
...
export function AccountImportDialog({ disabled, onImported }: AccountImportDialogProps) {
  ...
  const handleJsonSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const results = await Promise.all(
        files.map(async (file) => {
          ...
          const token = getAccountAccessToken(parsed);
          const record =
            parsed && typeof parsed === "object"
              ? ({ ...(parsed as Record<string, unknown>), import_file_name: file.name } as AccountImportRecord)
              : null;
          return {
            token,
            record,
      ...
      const records = results
        ...
        .filter((item): item is AccountImportRecord => Boolean(item && getAccountAccessToken(item)));
      const parsedFileCount = records.length;
      const errorCount = results.length - parsedFileCount;

      if (parsedFileCount === 0) {
        toast.error("这些 JSON 文件里没有读取到可用 access_token");
        return;
      }
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-5">
            <div className="space-y-2">
              <div className="text-sm font-medium text-stone-800">多选本地账号 JSON 文件</div>
          ...
          <input
            ...
            multiple
            className="hidden"
            onChange={(event) => void handleJsonSelected(event)}
          />
          {pendingJsonImport ? (
            <div className="rounded-2xl border border-stone-200 bg-white p-4 text-sm leading-6 text-stone-600">
              最近一次读取到 {pendingJsonImport.parsedFileCount} 个 Token
              {pendingJsonImport.errorCount > 0 ? `，另有 ${pendingJsonImport.errorCount} 个文件未提取成功` : ""}。
            </div>
          ) : null}
        </div>
      );
            ...
            {method === "json" ? (
              <Button
                className={cn(
                  ...
                  !pendingJsonImport ? "hidden" : "",
                )}
                onClick={() => setConfirmOpen(true)}
                disabled={footerDisabled || !pendingJsonImport}
              >
                查看导入确认
              </Button>
            ) : null}
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {pendingJsonImport
                ? `确认识别到 ${pendingJsonImport.parsedFileCount} 个账号，是否确认导入？`
                : "尚未读取到可导入的账号。"}
              {pendingJsonImport?.errorCount
                ? `，另有 ${pendingJsonImport.errorCount} 个文件未提取成功。`
                : "。"}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="pt-2">
            ...
            <Button
              ...
              disabled={isSubmitting || !pendingJsonImport}
            >
              {isSubmitting ? <LoaderCircle className="size-4 animate-spin" /> : null}
              确认导入
            </Button>
