...
const ACTIVE_CONVERSATION_STORAGE_KEY = "gpt2api-image:image_active_conversation_id";
const IMAGE_SIZE_STORAGE_KEY = "gpt2api-image:image_last_size";
...
function mapImageTasksByLookupId(tasks: ImageTask[]) {
  const taskMap = new Map<string, ImageTask>();
  for (const task of tasks) {
    taskMap.set(task.id, task);
    if (task.client_task_id) {
      taskMap.set(task.client_task_id, task);
    }
  }
  return taskMap;
}
...
async function syncConversationImageTasks(items: ImageConversation[]) {
  ...
  const taskMap = mapImageTasksByLookupId(taskList.items);
  let changed = false;
...
function ImagePageContent({ isRootAdmin }: { isRootAdmin: boolean }) {
  ...
  useEffect(() => {
    ...
    try {
      raw = window.sessionStorage.getItem("gpt2api-image:redraw_handoff");
      if (raw) window.sessionStorage.removeItem("gpt2api-image:redraw_handoff");
    } catch {
      return;
    }
    if (!raw) return;
  ...
  const runConversationQueue = useCallback(
    async (conversationId: string) => {
      ...
      const applyTasks = async (tasks: ImageTask[]) => {
        const taskMap = mapImageTasksByLookupId(tasks);
